...
namespace DetectorVirus.Data
{
    public class Analyzer
    {
        ...
        {
            byte[] posibleVirus = Possible_Virus.getSequence();
            Stack<Virus> foundVirus = new Stack<Virus>();

            // Recorremos la base de datos de virus
            for (int i = 0; i < Virus_DB.Length; i++)
            {
                var knownSequence = Virus_DB[i].getSequence();

                // Para cada virus de la base de datos, comprobamos si esa secuencia está contenida en la del archivo
                if (ByteArrayRocks.Search(posibleVirus, knownSequence) != -1)
                {
                    foundVirus.Push(Virus_DB[i]);
                }
            }

            return Task.FromResult(foundVirus.ToArray());
        }
    }
